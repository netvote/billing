...
const getCatalogSummary = async () => {
    ...
    plans.data.forEach((p) => {
        ...
        if (productId && (p.metadata.available === "true")) {
            if (!items[productId]) {
                items[productId] = {}
            }
...
const getSupportPlanChanges = async (customerId, level) => {
    ...
    else if ((planSubItem.plan.id !== catalog[PLAN_SUPPORT][level].planId)) {
        //modify
        changes.push({
            id: planSubItem.id,
            plan: catalog[PLAN_SUPPORT][level].planId
...
const getUsagePlanChanges = async (customerId, level) => {
    ...
    } else if (!meteredSubscriptionItem || (meteredSubscriptionItem.plan.id != catalog[PLAN_USAGE][level].meteredPlanId)) {
        changes.push({
    ...
    } else if (!usagePlanSubItem || (usagePlanSubItem.plan.id != catalog[PLAN_USAGE][level].planId)) {
        changes.push({
...
const setPlanLevels = async (stripeCustomerId, supportLevel, usageLevel) => {
    ...
    if (changes.length > 0) {
        if (customer.subscriptions.total_count === 0) {
            console.log(`adding subscription to ${stripeCustomerId}: ` + JSON.stringify(changes, null, 4))
            await stripe.subscriptions.create({
                customer: stripeCustomerId,
                items: changes,
                billing_cycle_anchor: Math.floor(moment().add(1,'months').startOf('month').valueOf()/1000)
            })
        } else {
